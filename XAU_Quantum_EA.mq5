...
int OnInit()
{
   printf("XAU_Quantum_EA: Starting OnInit...");

   //--- Initialize CSymbolInfo
   if(!symbolInfo.Name(_Symbol))
   {
       printf("Error: CSymbolInfo.Name() failed for symbol %s. Error code: %d", _Symbol, GetLastError());
       return(INIT_FAILED);
   }
   printf("OnInit: CSymbolInfo initialized.");
   ...
   {
       printf("Error: CTrade.SetTypeFillingBySymbol() failed. Error code: %d", GetLastError());
       // This is not a fatal error, so we can continue.
   }
   printf("OnInit: CTrade initialized.");
   ...
   printf("XAU_Quantum_EA Initialized Successfully.");
